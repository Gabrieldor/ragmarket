...
def test_parse_plain_number():
    item_name, operator, target_price, required_refine, required_slot = parse_rule("Elunium > 30000")
...
def test_parse_k_suffix():
    item_name, operator, target_price, required_refine, required_slot = parse_rule("Elunium > 30k")
...
def test_parse_kk_suffix():
    item_name, operator, target_price, required_refine, required_slot = parse_rule("Oridecon < 5kk")
...
def test_parse_item_name_with_spaces():
    item_name, _, _, required_refine, required_slot = parse_rule("Tiara Carnavalesca < 20kk")
...
def test_parse_decimal_price():
    _, _, target_price, required_refine, required_slot = parse_rule("Elunium > 1.5k")
