...
class DiscordNotifier:
    ...
    Messages are queued internally so that the monitor never blocks waiting
    for Discord's rate-limiter.  The discord.Client runs as a background
    asyncio task sharing the same event loop as the monitor.
    """
    ...
    def _condition_line(self, rule: WatchRule) -> str:
        symbol = ">=" if rule.operator == ">" else "<="
        return f"{rule.item_name} {symbol} {_fmt(rule.target_price)}"

    async def send_triggered(self, rule: WatchRule, price: int) -> None:
        msg = (
            f"{self.user_mention}\n\n"
    ...
    async def send_cleared(self, rule: WatchRule) -> None:
        msg = (
            f"{self.user_mention}\n\n"
    ...
    async def send_price_changed(
        self, rule: WatchRule, old_price: Optional[int], new_price: int
    ) -> None:
