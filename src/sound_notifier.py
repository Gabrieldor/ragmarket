"""Local sound notifier — plays beeps instead of sending Discord messages.

Enabled by setting ``"local_sound": true`` in config.json.
Uses Python's built-in ``winsound`` module (Windows only; no extra deps).
